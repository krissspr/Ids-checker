  ...
  try {
    ...
    for (const m of models) {
      ...
      try {
        ...
        // Force full snapshot – no lazy {…} in console
        const fileKeys = file ? Object.keys(file) : [];
        const fileSnapshot = {};
        fileKeys.forEach(k => { try { fileSnapshot[k] = file[k]; } catch(e) {} });
        log.info("FILE KEYS:", fileKeys.join(", "));
        log.info("FILE SNAPSHOT:", JSON.stringify(fileSnapshot));
        if (file?.name?.toLowerCase().endsWith(".ifc")) {
          const fileId = file.id || file.fileId || file.fileVersionId || file.versionId || modelId;
          log.info(`FILE IDs → id:${file.id} | fileId:${file.fileId} | versionId:${file.versionId} | using:${fileId}`);
          ifcModels.push({ modelId, name: file.name, fileId, size: file.size });
          log.ok(`IFC: ${file.name} → fileId: ${fileId}`);
        }
...
function PropertyEditor({ spec, model, tc, devMode, onBack }) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      log.info("TC region:", region, "project location:", project?.location);
  ...
  const handleRun = async () => {
    ...
    try {
      ...
      if (ifcTab === "viewer" && selectedModel && token && !devMode) {
        ...
        log.info("Run TC region:", runRegion);
