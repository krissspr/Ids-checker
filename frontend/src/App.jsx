                  ...
                  <div key={ri} style={{marginBottom: ri < reqs.length - 1 ? 10 : 0}}>
                    ...
                    {failing.slice(0, 50).map((f, fi) => (
                      <div key={fi}
                        ...
                        style={{ display:"flex", gap:8, alignItems:"center", padding:"4px 6px", paddingLeft:12, borderRadius:3, borderBottom:fi < Math.min(failing.length, 50) - 1 ? `1px solid ${M.grayLight}` : "none", cursor:(selectMode||onMark)&&f.guid?"pointer":"default", transition:"background 0.1s" }}
            ...
            return (
              <>
                ...
                {spec.failures.map((f, i) => (
                  <div key={i}
                    onClick={() => { if (selectMode) { f.guid && toggleSelected(f.guid); } else { onMark && f.guid && onMark([f.guid]); } }}
                    style={{ display:"flex", gap:8, alignItems:"center", padding:"5px 6px", borderRadius:3, borderBottom:i<spec.failures.length-1?`1px solid ${M.grayLight}`:"none", cursor:(selectMode||onMark)&&f.guid?"pointer":"default", transition:"background 0.1s" }}
                    onMouseEnter={e => { if ((selectMode||onMark)&&f.guid) e.currentTarget.style.background = M.bluePale; }}
                    onMouseLeave={e => { e.currentTarget.style.background = "transparent"; }}
                  >
                    {selectMode && f.guid && (
