    ...
    for (const m of models) {
      ...
      try {
        ...
        if (file?.name?.toLowerCase().endsWith(".ifc")) {
          // Log all available ID fields so we can find the right one
          const fileId = file.id || file.fileId || file.fileVersionId || file.versionId || modelId;
          log.info(`IFC file IDs – id:${file.id} fileId:${file.fileId} versionId:${file.versionId} modelId:${modelId}`);
          ifcModels.push({ modelId, name: file.name, fileId, size: file.size, _rawFile: file });
          log.ok(`IFC found: ${file.name} → using fileId: ${fileId}`);
        }
