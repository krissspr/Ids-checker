...
function PropertyEditor({ spec, model, tc, devMode, onBack }) {
  // Build initial values from requirements_detail
  ...
  const allFilled = requirements.length === 0
    ? false
    : requirements.every((_, i) => (values[i] || "").trim().length > 0);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      log.info("region:", region, "project:", project?.id);

      // Build requirements array
      const reqArray = requirements.map((req, i) => ({
        pset: req.pset || "",
        name: req.name,
        value: values[i] || "",
      }));
  ...
  return (
    <div style={{ flex:1, overflow:"auto", display:"flex", flexDirection:"column" }}>
      ...
      <div style={{ padding:14, display:"flex", flexDirection:"column", gap:14, flex:1, overflow:"auto" }}>
        ...
        {/* Save button */}
        <button disabled={!allFilled || saving || requirements.length === 0} onClick={handleSave} style={{
          padding:"10px 0", borderRadius:4, border:"none",
          cursor: allFilled && !saving && requirements.length > 0 ? "pointer" : "not-allowed",
          background: allFilled && !saving && requirements.length > 0 ? M.blue : M.gray1,
          color: allFilled && !saving && requirements.length > 0 ? M.white : M.gray6,
          ...
          {ifcTab === "viewer" ? (
            loadingModels
              ...
              : loadedModels.length === 0
                ? <div style={{background:M.yellowPale,border:`1px solid ${M.yellow}`,borderRadius:4,padding:10,fontSize:12,color:M.gray8}}><strong>Ingen IFC-modeller i viewer.</strong><br/>Åpne en modell eller last opp manuelt.</div>
                : <>
