...
export default function IDSChecker() {
  ...
  const createTodo = async (spec, title, description) => {
    ...
    try {
      ...
      log.info("Creating todo:", title, "guids:", spec.failures.length);
      ...
      <div style={{ flex:1, overflow:"auto", padding:14, display:"flex", flexDirection:"column", gap:14 }}>
        ...
        {results && (
          <div style={{animation:"fadeUp 0.3s ease"}}>
            ...
            {specs.map((spec, i) => (
              <SpecRow key={spec.name} spec={spec} index={i} onMark={canMark?handleMark:null} canMark={canMark} onEditProps={setEditingSpec} onCreateTodo={tc && !devMode ? (spec, title, desc) => createTodo(spec, title, desc) : null}/>
            ))}
          </div>
        )}
      </div>
