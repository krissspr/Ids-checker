...
// ── ToDo Creator ─────────────────────────────────────────────────────────────
function TodoCreator({ specs, tc, model }) {
  ...
  // Load project members when opened
  const handleOpen = async () => {
    setOpen(true);
    setResult(null);
    if (members.length > 0) return;
    setLoadingMembers(true);
    log.group("loadMembers");
    try {
      const token = tc.getAccessToken();
      const project = await tc.api.project.getCurrentProject();
      const region = project?.location === "europe" ? "app.eu" : "app";
      log.info("Loading members for project:", project?.id);

      const res = await fetch(
        `${API_BASE}/project-members?tc_project_id=${project.id}&tc_access_token=${token}&tc_region=${region}`,
      );
      if (res.ok) {
        const data = await res.json();
        log.ok("Members:", data);
        ...
        log.warn("Could not load members:", res.status);
      }
    } catch (e) {
      log.error("loadMembers failed:", e.message);
    } finally {
  ...
  const handleCreate = async () => {
    setCreating(true);
    setResult(null);
    log.group("createTodos");
    try {
      const token = tc.getAccessToken();
      const project = await tc.api.project.getCurrentProject();
      const region = project?.location === "europe" ? "app.eu" : "app";

      const todoList = specs.map(spec => ({
        title: `IDS: ${spec.name}`,
        description: buildDescription(spec),
        guids: spec.failures.map(f => f.guid).filter(Boolean),
        modelId: model?.modelId || model?.fileId || "",
      }));
      log.info("Creating", todoList.length, "todos");
      ...
      form.append("tc_project_id", project.id);
      if (model?.tcHost) form.append("tc_host", model.tcHost);
      form.append("todos", JSON.stringify(todoList));
      if (assigneeId) form.append("assignee_id", assigneeId);
      log.info("tc_host:", model?.tcHost, "project:", project.id);
      ...
      log.error("createTodos failed:", e.message);
  ...
  const buildDescription = (spec) => {
    const lines = [
      `Regel: ${spec.name}`,
      `Gjelder: ${spec.applicability}`,
      `Krav: ${spec.requirement}`,
      ``,
      `Feilet (${spec.failed} av ${spec.total}):`,
      ...spec.failures.slice(0, 20).map(f => `• ${f.name} (${f.type})`),
      spec.more_failures > 0 ? `• … og ${spec.more_failures} til` : "",
    ];
...
");
  ...
  return (
    <div style={{ background:M.white, border:`1px solid ${M.gray0}`, borderRadius:4, padding:12, marginBottom:12, animation:"fadeUp 0.3s ease" }}>
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom: open ? 12 : 0 }}>
        <div style={{ fontSize:12, fontWeight:600, color:M.gray }}>
          Opprett ToDo i TC
        </div>
        <button onClick={open ? () => setOpen(false) : handleOpen} style={{ fontSize:11, padding:"4px 10px", borderRadius:3, border:`1px solid ${M.blue}`, background:open?M.bluePale:M.white, color:M.blue, cursor:"pointer", fontFamily:"inherit", fontWeight:600 }}>
          {open ? "Lukk" : `Opprett ${specs.length} ToDo${specs.length !== 1 ? "er" : ""}`}
        </button>
      </div>
      ...
      {open && (
        <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
          ...
          {/* Preview of todos to be created */}
          <div style={{ background:M.grayLight, borderRadius:4, padding:10, maxHeight:140, overflowY:"auto" }}>
            <div style={{ fontSize:10, fontWeight:700, color:M.gray6, marginBottom:6, textTransform:"uppercase", letterSpacing:"0.06em" }}>
              Disse ToDo-ene opprettes:
            </div>
            {specs.map((spec, i) => (
              <div key={i} style={{ display:"flex", gap:6, alignItems:"center", padding:"3px 0", fontSize:11, color:M.gray }}>
                <div style={{ width:5, height:5, borderRadius:"50%", background:M.red, flexShrink:0 }}/>
                <span style={{ fontWeight:600 }}>IDS: {spec.name}</span>
                <span style={{ color:M.gray6 }}>– {spec.failed} objekt{spec.failed !== 1 ? "er" : ""}</span>
              </div>
            ))}
          </div>
          ...
          <div>
            <label style={{ fontSize:11, fontWeight:600, color:M.gray8, display:"block", marginBottom:4 }}>
              Tildel til <span style={{ fontWeight:400, color:M.gray6 }}>(valgfritt)</span>
            </label>
            {loadingMembers ? (
              <div style={{ fontSize:11, color:M.gray6, display:"flex", gap:6, alignItems:"center" }}>
                <Icon.Spinner/> Laster prosjektmedlemmer…
              </div>
            ) : (
              <select
                value={assigneeId}
                onChange={e => setAssigneeId(e.target.value)}
                style={{ width:"100%", padding:"8px 10px", fontSize:12, borderRadius:4, border:`1px solid ${M.gray1}`, fontFamily:"inherit", color:M.gray, background:M.white, cursor:"pointer" }}
              >
          ...
          <button onClick={handleCreate} disabled={creating} style={{
            padding:"9px 0", borderRadius:4, border:"none",
            cursor: creating ? "not-allowed" : "pointer",
            background: creating ? M.gray1 : M.blue,
            color: creating ? M.gray6 : M.white,
            fontFamily:"inherit", fontSize:12, fontWeight:600,
            display:"flex", alignItems:"center", justifyContent:"center", gap:8,
          }}>
            {creating
              ? <><Icon.Spinner color={M.white}/> Oppretter ToDo-er…</>
              : `Opprett ${specs.length} ToDo${specs.length !== 1 ? "er" : ""} i TC`
            }
          </button>

          {/* Result */}
          {result && (
            <div style={{ padding:"8px 10px", borderRadius:4, fontSize:11, lineHeight:1.6,
              background: result.error || result.failed > 0 ? M.redPale : M.greenPale,
              border: `1px solid ${result.error || result.failed > 0 ? M.red : M.green}`,
              color: result.error || result.failed > 0 ? M.redDark : M.greenDark,
            }}>
              {result.error
                ? `✕ ${result.error}`
                : <>
                    <strong>✓ {result.created} ToDo{result.created !== 1 ? "er" : ""} opprettet</strong>
                    {result.failed > 0 && <><br/>{result.failed} feilet</>}
                  </>
              }
            </div>
          )}
...
function SpecRow({ spec, index, onMark, canMark, onEditProps }) {
  ...
  // Check if spec has editable requirements
    ...
    <div style={{ fontFamily:"'Open Sans','Roboto',sans-serif", background:M.grayLight, minHeight:"100vh", color:M.gray, display:"flex", flexDirection:"column" }}>
      ...
      <div style={{ flex:1, overflow:"auto", padding:14, display:"flex", flexDirection:"column", gap:14 }}>
        ...
        <section>
          ...
          {ifcTab === "viewer" ? (
            <div style={{background:M.yellowPale, border:`1px solid ${M.yellowDark}`, borderRadius:4, padding:12, fontSize:12, color:M.gray8, lineHeight:1.7}}>
              <strong>⚠ Ikke tilgjengelig</strong><br/>
              Trimble Connect konverterer IFC-filer til TRIMBIM-format ved opplasting. Originalfilen kan ikke lastes ned direkte via API.<br/><br/>
              <strong>Løsning:</strong> Bruk fanen <em>Last opp</em> og velg IFC-filen fra din PC.
            </div>
          ) : (
        ...
        {results && (
          <div style={{animation:"fadeUp 0.3s ease"}}>
            ...
            {/* ToDo creator */}
            {results.summary.failed > 0 && !devMode && tc && (
              <TodoCreator specs={results.specifications.filter(s => s.status === "failed")} tc={tc} model={selectedModel} />
            )}
