...
async function detectLoadedModels(api) {
  ...
  try {
    ...
    for (const m of models) {
      log.info("Full model entry:", JSON.stringify(m));
      const modelId = m.modelId || m.id || m.fileId || m.versionId;
      if (!modelId) continue;
      try {
        const file = await api.viewer.getLoadedModel(modelId);
        log.info("FILE KEYS:", file ? Object.keys(file).join(", ") : "null");
        log.info("FILE SNAPSHOT:", JSON.stringify(file));

        // TC wraps the original file under file.file – use that for download
        const innerFile = file?.file || file;
        const fileName = innerFile?.name || file?.name || "";

        if (fileName.toLowerCase().endsWith(".ifc")) {
          const fileId = innerFile?.id || innerFile?.versionId || file?.id || file?.versionId || modelId;
          // Extract server host from thumbnailUrl if present – TC may use a specific server
          let tcHost = null;
          const thumb = innerFile?.thumbnailUrl?.[0] || innerFile?.thumbnailUrl;
          if (thumb) {
            try { tcHost = new URL(thumb).host; } catch(e) {}
          }
          log.info(`FILE IDs → innerFile.id:${innerFile?.id} | versionId:${innerFile?.versionId} | using:${fileId} | host:${tcHost}`);
          ifcModels.push({ modelId, name: fileName, fileId, size: innerFile?.size, tcHost });
          log.ok(`IFC: ${fileName} → fileId: ${fileId} host: ${tcHost}`);
        }
...
function PropertyEditor({ spec, model, tc, devMode, onBack }) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Frontend downloads IFC and sends to backend – Railway has no outbound network
      log.info("Downloading IFC for property update, fileId:", model?.fileId, "host:", model?.tcHost);
      const host = model?.tcHost || "app.eu.connect.trimble.com";
      const dlUrl = `https://${host}/tc/api/2.0/files/${model?.fileId}/download`;
      const ifcRes = await fetch(dlUrl, { headers: { Authorization: `Bearer ${token}` } });
      log.info("IFC download:", ifcRes.status);
      if (!ifcRes.ok) throw new Error(`Kunne ikke laste ned IFC: ${ifcRes.status}`);
      const ifcBlob = await ifcRes.blob();
      log.ok("Downloaded:", ifcBlob.size, "bytes");
      form.append("ifc_file", new File([ifcBlob], model?.name || "model.ifc"));
  ...
  const handleRun = async () => {
    ...
    try {
      ...
      if (ifcTab === "viewer" && selectedModel && token && !devMode) {
        // Frontend downloads IFC from TC and sends to backend
        // This avoids Railway needing outbound network access to TC
        setLoadingStep("Laster ned IFC fra TC…");
        log.info("Downloading IFC from TC, fileId:", selectedModel.fileId, "host:", selectedModel.tcHost);

        const host = selectedModel.tcHost || "app.eu.connect.trimble.com";
        const downloadUrl = `https://${host}/tc/api/2.0/files/${selectedModel.fileId}/download`;
        log.info("Download URL:", downloadUrl);

        const ifcRes = await fetch(downloadUrl, {
          headers: { Authorization: `Bearer ${token}` },
        });
        log.info("IFC download response:", ifcRes.status);

        if (!ifcRes.ok) {
          throw new Error(`Kunne ikke laste ned IFC fra TC: ${ifcRes.status}`);
        }

        const ifcBlob = await ifcRes.blob();
        log.ok("IFC downloaded:", ifcBlob.size, "bytes");
        const ifcFile = new File([ifcBlob], selectedModel.name, { type: "application/octet-stream" });
