...
async def _download_ifc_from_tc(
    ...
    async with httpx.AsyncClient(timeout=180) as client:
        ...
        # Try multiple TC API endpoint patterns for listing folder contents
        if tc_parent_id and tc_file_name:
            folder_url_patterns = [
                f"{base_url}/projects/{tc_project_id}/nodes?parent={tc_parent_id}&limit=100",
                f"{base_url}/projects/{tc_project_id}/files?folderId={tc_parent_id}&limit=100",
                f"{base_url}/folders/{tc_parent_id}/contents",
                f"{base_url}/folders/{tc_parent_id}/nodes",
            ]

            for folder_url in folder_url_patterns:
                print(f"Trying folder URL: {folder_url}", flush=True)
                folder_res = await client.get(folder_url, headers=headers)
                print(f"  → {folder_res.status_code} {folder_res.text[:200]}", flush=True)

                if folder_res.status_code == 200:
                    data = folder_res.json()
                    # TC may return list in different keys
                    files = (data.get("list") or data.get("files") or
                             data.get("nodes") or data.get("items") or
                             (data if isinstance(data, list) else []))
                    print(f"  → {len(files)} items found", flush=True)
                    ...
                    for f in files:
                        ...
                        print(f"  File: {name} type:{ftype} id:{fid}", flush=True)
                        ...
                        if name == target and "TRIMBIM" not in ftype:
                            ...
                            if dl.status_code == 200:
                                with tempfile.NamedTemporaryFile(suffix=".ifc", delete=False) as tmp:
                                    ...
                                    return tmp.name
                    break  # Found a working endpoint, stop trying
