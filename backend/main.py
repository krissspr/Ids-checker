    ...
    async with httpx.AsyncClient(timeout=60) as client:
        for todo in todo_list:
            ...
            if todo_id and guids and model_id:
                # Create a View with the failing objects highlighted
                # and link it to the ToDo via todoId field.
                # This is how TC itself links views to todos.
                view_body = {
                    "name": f"IDS feil: {todo.get('title', '')}",
                    "description": "Automatisk generert av IDS Regelsjekker",
                    "projectId": tc_project_id,
                    "models": [model_id],
                    "files": [model_id],
                    "todoId": todo_id,
                ...
                # Also create objectlinks – confirmed structure from Network tab
                # data needs both objectId and sourceId
                link_body = {
                    "source": {
                        "id": model_id,
                        "versionId": model_id,
