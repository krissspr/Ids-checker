...
async def _download_ifc_from_tc(
    ...
    """
    # Use tc_host for file downloads (app21 = storage server)
    # Use tc_region for API calls (app.eu = API gateway with correct endpoints)
    if tc_host:
        download_base = f"https://{tc_host}/tc/api/2.0"
    else:
        download_base = f"https://{tc_region}.connect.trimble.com/tc/api/2.0"

    api_base = f"https://{tc_region}.connect.trimble.com/tc/api/2.0"
    base_url = api_base  # default for backwards compat

    headers = {"Authorization": f"Bearer {tc_access_token}"}
    print(f"API base: {api_base} | Download base: {download_base}", flush=True)
    print(f"file: {tc_file_name} | folder: {tc_parent_id} | project: {tc_project_id}", flush=True)

    async with httpx.AsyncClient(timeout=180) as client:

        # All API calls go to download_base (app21) – this is the server
        # that actually has the files AND responds to API calls for this project.
        # app.eu is the load balancer but projects may be pinned to specific pods.

        # Strategy 1: List folder using confirmed TC endpoint: /folders/{id}
        if tc_parent_id and tc_file_name:
            # Confirmed TC endpoint for folder contents
            folder_url = f"{download_base}/folders/{tc_parent_id}/items?tokenThumburl=false&sort=+name"
            print(f"Folder items: {folder_url}", flush=True)
            try:
                r = await client.get(folder_url, headers=headers)
                print(f"  → {r.status_code} | {r.text[:500]}", flush=True)
                if r.status_code == 200:
                    data = r.json()
                    print(f"  Keys: {list(data.keys()) if isinstance(data, dict) else type(data)}", flush=True)
                    files = (data.get("list") or data.get("items") or
                             data.get("files") or data.get("nodes") or
                             (data if isinstance(data, list) else []))
                    target = tc_file_name.lower()
                    print(f"  {len(files)} items in folder", flush=True)
                    for f in files:
                        print(f"    ITEM: {f}", flush=True)
                        name = (f.get("name") or "").lower()
                        ftype = (f.get("type") or f.get("runtimeType") or "").upper()
                        fid = f.get("id") or f.get("versionId")
                        if name == target and "TRIMBIM" not in ftype:
                            dl = await client.get(
                                f"{download_base}/files/{fid}/download",
        ...
        # Strategy 2: Direct download by file ID
        print(f"Fallback: direct download {tc_file_id} from {download_base}", flush=True)
        res = await client.get(
            f"{download_base}/files/{tc_file_id}/download",
            headers=headers,
