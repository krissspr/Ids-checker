...
async def _download_ifc_from_tc(tc_file_id: str, tc_access_token: str, tc_region: str) -> str:
...
async def validate(
    ...
    try:
        ...
        if tc_file_id and tc_access_token:
            ifc_path = await _download_ifc_from_tc(tc_file_id, tc_access_token, tc_region)
        else:
            with tempfile.NamedTemporaryFile(suffix=".ifc", delete=False) as tmp:
