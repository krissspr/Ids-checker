    ...
    async with httpx.AsyncClient(timeout=60) as client:
        for todo in todo_list:
            ...
            if todo_id and guids and model_id:
                # Confirmed TC objectlink structure from Network tab:
                # source.data = [{ sourceId: GUID, type: "BIMOBJECT" }]
                # All GUIDs in one request
                link_body = {
                    "source": {
                        "id": model_id,
                        "versionId": model_id,
                ...
                print(f"  Objectlinks ({len(guids[:50])} objects): {link_res.status_code} {link_res.text[:200]}", flush=True)
