...
async def _download_ifc_from_tc(
    ...
    async with httpx.AsyncClient(timeout=180) as client:
        ...
        # Strategy 1: Find original IFC in parent folder by name
        # TC API 2.0 uses /projects/{id}/nodes?parent={folderId} structure
        if tc_parent_id and tc_file_name and tc_project_id:
            folder_url_patterns = [
                f"{base_url}/projects/{tc_project_id}/nodes?parent={tc_parent_id}&limit=500",
                f"{base_url}/projects/{tc_project_id}/nodes?parentId={tc_parent_id}&limit=500",
                f"{base_url}/projects/{tc_project_id}/nodes/{tc_parent_id}",
                f"{base_url}/projects/{tc_project_id}/files?parent={tc_parent_id}&limit=500",
                f"{base_url}/nodes?projectId={tc_project_id}&parent={tc_parent_id}&limit=500",
            ]

            for folder_url in folder_url_patterns:
                print(f"Trying: {folder_url}", flush=True)
                folder_res = await client.get(folder_url, headers=headers)
                print(f"  → {folder_res.status_code} | {folder_res.text[:300]}", flush=True)

                if folder_res.status_code == 200:
                    data = folder_res.json()
                    # Flatten whatever structure TC returns
                    ...
                    for f in files:
                        ...
                        fid = f.get("id") or f.get("versionId") or f.get("identifier")
                        print(f"    {name} | type:{ftype} | id:{fid}", flush=True)

                        if name == target and "TRIMBIM" not in ftype:
                            print(f"  Found original IFC: {name} id:{fid}", flush=True)
                            dl = await client.get(
