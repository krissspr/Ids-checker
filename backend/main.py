...
async def _download_ifc_from_tc(
    ...
    async with httpx.AsyncClient(timeout=180) as client:
        ...
        if tc_parent_id and tc_file_name:
            print(f"Listing folder {tc_parent_id}...", flush=True)
            folder_res = await client.get(
                f"{base_url}/folders/{tc_parent_id}/files",
                headers=headers,
            )
            print(f"Folder response: {folder_res.status_code}", flush=True)

            if folder_res.status_code == 200:
                files = folder_res.json().get("list") or folder_res.json().get("files") or []
                print(f"Files in folder: {len(files)}", flush=True)
