...
async def _download_ifc_from_tc(
    ...
    async with httpx.AsyncClient(timeout=180) as client:
        ...
        if tc_parent_id and tc_file_name:
            folder_url = f"{download_base}/folders/{tc_parent_id}"
            print(f"Folder listing: {folder_url}", flush=True)
            try:
                r = await client.get(folder_url, headers=headers)
                print(f"  → {r.status_code} | {r.text[:500]}", flush=True)
                if r.status_code == 200:
                    data = r.json()
                    # Log all keys to understand response structure
                    print(f"  Response keys: {list(data.keys()) if isinstance(data, dict) else 'list'}", flush=True)
                    files = (data.get("list") or data.get("files") or
                             data.get("nodes") or data.get("items") or
                             data.get("children") or
                             (data if isinstance(data, list) else []))
                    target = tc_file_name.lower()
                    print(f"  {len(files)} items", flush=True)
                    for f in files:
                        name = (f.get("name") or "").lower()
                        ftype = (f.get("type") or f.get("runtimeType") or "").upper()
                        fid = f.get("id") or f.get("versionId")
                        print(f"  {name} | {ftype} | {fid}", flush=True)
